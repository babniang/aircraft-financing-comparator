      ...
      <table className="xl">
        ...
        <tbody>
          {data.results.map((r) => {
            ...
            const isCheapest = r.structure === data.cheapest;
            return (
              <tr key={r.structure} className={isCheapest ? "xl-hi" : undefined}>
                <td>{t(`structure.${r.structure}`)}</td>
                <td>{num(r.implied_annual_cost_pct, 2)}%</td>
